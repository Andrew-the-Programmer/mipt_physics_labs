...
def calculate_value_r(equation, *, single_subs: dict = None, range_subs: dict = None):
    equation = sp.simplify(equation)
    logging.debug(f"calculate_value:\n{equation=}\n{single_subs=}\n{range_subs=}")
    equation = equation.evalf(subs=single_subs)
    equation = sp.simplify(equation)
    if not range_subs:
        return equation
    return [calculate_value_r(equation, single_subs=sub) for sub in f(range_subs)]


def calculate_value(equation, subs: dict[sp.Symbol | str, Qe | float]):
    logging.debug(f"calculate_value: {equation=}\n{subs=}")
    normal_subs: dict = dict()
    stack_subs: dict = dict()
    ...
    res = equation.evalf(subs=normal_subs)
    res = sp.simplify(res)

    logging.debug(normal_subs)
    logging.debug(stack_subs)
    ...
    for symbol, values in stack_subs.items():
        for i, value in enumerate(values):
            if i < len(new_subs):
                new_subs[i][symbol] = value
            else:
                new_subs.append({symbol: value})

    print(new_subs)

    return [calculate_value(res, subs=sub) for sub in new_subs]
...
def calculate_err(equation, subs: dict[sp.Symbol | str, Qe | float]):
    ...
    for symbol in subs.keys():
        new_subs[get_err_symbol(symbol)] = get_err(subs[symbol])
    err_eq = sp.simplify(err_eq)
    # res = calculate_value(err_eq, {})
    logging.debug(f"{new_subs=}")
    return calculate_value(err_eq, new_subs)
...
def calculate_r(equation, single_subs: dict[sp.Symbol | str, Qe | float]) -> Qe:
    logging.debug(f"calculate: {equation=}\n{subs=}")
    value = calculate_value(equation, subs)
    err = calculate_err(equation, subs)
    return Qe(value=value, err=err)
...
if __name__ == "__main__":
    ...
    print(*l)
    for v in l:
        print(dict(zip(d.keys(), v)))
    exit(0)
    x, y = sp.symbols("x y")
    d = {x: 1, y: 2}
    print(d)
    logging.basicConfig(level=logging.DEBUG)
    # print(calculate(x + y * sp.exp(x * y), subs={x: [1, 2, 3], y: 2}))
